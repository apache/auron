...
impl Agg for AggLast {
    ...
    fn partial_update(
        ...
        let accs = downcast_any!(accs, mut AccLastColumn)?;
        accs.ensure_size(acc_idx);

        let (value_accs, flag_accs) = accs.inner_mut();

        // last() keeps the latest visited row (including null), so every row
        // unconditionally overwrites the accumulator.
    ...
    fn partial_merge(
        ...
        let accs = downcast_any!(accs, mut AccLastColumn)?;
        let merging_accs = downcast_any!(merging_accs, mut AccLastColumn)?;
        accs.ensure_size(acc_idx);

        let (value_accs, flag_accs) = accs.inner_mut();
        let (merging_value_accs, merging_flag_accs) = merging_accs.inner_mut();

        // the merging accumulator carries later-visited rows, so it overwrites
        // whenever it has visited any row.
            ...
            ($ty:ty) => {{
                ...
                idx_for_zipped! {
                    ((acc_idx, merging_acc_idx) in (acc_idx, merging_acc_idx)) => {
                        if merging_flag_accs.value(merging_acc_idx).is_some() {
...
impl AccColumn for AccLastColumn {
    fn as_any(&self) -> &dyn Any {
        self
    }
