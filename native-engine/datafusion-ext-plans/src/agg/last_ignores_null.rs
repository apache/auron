...
impl Debug for AggLastIgnoresNull {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "LastIgnoresNull({:?})", self.child)
    }
}

impl Agg for AggLastIgnoresNull {
    ...
    fn partial_update(
        ...
        // last(ignoreNulls=true) keeps the latest visited non-null row, so every
        // non-null row unconditionally overwrites the accumulator.
    ...
    fn partial_merge(
        ...
        // the merging accumulator carries later-visited rows, so its non-null
        // value overwrites whatever was visited earlier.
