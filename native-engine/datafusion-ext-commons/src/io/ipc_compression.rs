...
mod tests {
    ...
    fn test_ipc_compression() -> Result<(), Box<dyn Error>> {
        ...
        let (num_rows1, arrays1) = reader.read_batch(&schema).expect("read_batch").expect("non-empty");
        assert_eq!(num_rows1, 2);
        assert_eq!(arrays1, &[test_array1]);
        let (num_rows2, arrays2) = reader.read_batch(&schema).expect("read_batch").expect("non-empty");
