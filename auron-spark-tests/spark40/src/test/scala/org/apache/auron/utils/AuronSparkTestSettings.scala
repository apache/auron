...
class AuronSparkTestSettings extends SparkTestSettings {
  ...
  enableSuite[AuronDataFrameFunctionsSuite]
    // Native make_array execution can crash on Spark 4.
    .exclude("DataFrame function and SQL function parity")
    ...
    // Native flatten can panic when child arrays have different containsNull metadata.
