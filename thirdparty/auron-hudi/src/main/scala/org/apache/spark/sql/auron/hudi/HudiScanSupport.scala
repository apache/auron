  ...
  private def tableTypeFromMeta(options: Map[String, String]): Option[String] = {
    ...
    basePath.flatMap { path =>
      try {
        ...
        case t: Throwable =>
          logInfo(s"Failed to load hudi table type from $path: ${t.getClass.getSimpleName}")
          None
      }
    }
  }
  ...
  private def baseFileFormatFromMeta(options: Map[String, String]): Option[String] = {
    ...
    basePath.flatMap { path =>
      try {
        ...
        case t: Throwable =>
          logInfo(s"Failed to load hudi base file format from $path: ${t.getClass.getSimpleName}")
          None
      }
    }
  }
