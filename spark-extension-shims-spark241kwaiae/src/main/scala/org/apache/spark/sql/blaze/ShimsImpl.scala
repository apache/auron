...
class ShimsImpl extends Shims with Logging {
  ...
  override def createNativeWindowExec(
    ...
    assert(groupLimit.isEmpty, "spark241kwaiae does not support WindowExec with groupLimit")
    assert(
      outputWindowCols,
      "spark241kwaiae does not support WindowExec without outputWindowCols")
  ...
  override def convertMoreExprWithFallback(
    ...
    e match {
      ...
      case PromotePrecision(_1) =>
        throw new NotImplementedError("PromotePrecision is disabled")
          ...
          case _ =>
            partition match {
              ...
              case p =>
                Iterator.single(
                  SparkEnv.get.shuffleManager
                    .getReader(shuffleHandle, p.index, p.index + 1, taskContext, metricsReporter))
            }
        }
        ...
        JniBridge.resourcesMap.put(
          ...
          () => {
            CompletionIterator[Object, Iterator[Object]](
              readers.flatMap(_.asInstanceOf[BlazeBlockStoreShuffleReaderBase[_, _]].readIpc()),
  ...
  def getPartitionedFile(
      partitionValues: org.apache.spark.sql.catalyst.InternalRow,
      filePath: String,
      offset: Long,
      size: Long): org.apache.spark.sql.execution.datasources.PartitionedFile =
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    plan.foreachUp {
      case p @ LogicalRelation(fsRelation: HadoopFsRelation, _, _, _) =>
        ...
        if (!(
          BlazeConverters.enableScanParquet && fsRelation.fileFormat.isInstanceOf[ParquetFileFormat]
            || BlazeConverters.enableScanOrc && fsRelation.fileFormat.isInstanceOf[OrcFileFormat]
        )) {
        ...
        if (p.catalogTable
          ...
          turnOffBlazeWithReason(p.conf, BlazeMissPatterns.ReadBlacklistedTable)
        }
